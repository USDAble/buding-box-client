...
describe('buildExportConversation', () => {
  // OCTO-FORK: the assistant label is the product name, resolved through the
  // i18n store. Guards the shape of the fix as much as the value: passing the
  // store object itself instead of its value throws inside brandShortName
  // (localize calls locale.split), so a passing assertion here also proves the
  // label is fed a real locale string.
  it('labels the assistant with the product name in the active language', () => {
    setLocale('en')
    expect(buildExportConversation([{ type: 'assistant_message', content: 'hi' }]))
      .toContain('>Pudding<')
    setLocale('zh')
    expect(buildExportConversation([{ type: 'assistant_message', content: 'hi' }]))
      .toContain('>布丁盒子<')
