# P0-02：客户端认证与控制面 Remote Adapter

| 项 | 内容 |
| --- | --- |
| 当前仓库主责 | 客户端 SDK/桌面组 |
| 中台协作方式 | 按[中台交付包](../backend抽象层与中台对接/中台交付包.md)提供 OpenAPI、sandbox、签名公钥和错误码；不在本仓库实现服务端 |
| 可并行 | 与 P0-03、P0-04、P0-06、P0-07 并行 |
| 依赖 | P0-01 的 DTO、profile 和 remote client 装配方式；正式凭证落盘依赖 P0-08 的 credential store |
| 需求基线 | [P0 正式上线需求基线](../P0-正式上线需求基线.md) R1、R3、R4、R8；冲突时先更新基线 |

## 目标

让生产客户端在一次真实登录后，透过 P0-01 定义的端口获得短期产品会话、账号/激活摘要、套餐/余额、模型目录版本、能力矩阵和词库版本。中台是外部控制面，不向客户端发送供应商 endpoint 或 API key。
...
1. 基于 P0-01 的 `AuthPort` / `ControlPlanePort` 实现 remote adapter；外部请求/响应类型仅在 adapter 内映射为客户端 DTO。
...
6. 在 sandbox 到位前，使用同一 OpenAPI fixture 的 fake adapter 做单元/集成测试；不得把临时 JSON 写进页面层。
...
字段合同由 P0-01 的 B0 先冻结并发给中台；当前仓库随后以 generated/fake client 开发 remote adapter。中台提供 OpenAPI + sandbox 后，通过 contract CI 才合真实 adapter；不要让页面团队直接使用临时 JSON。
