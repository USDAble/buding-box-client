...
| 适用范围 | Windows 便携版正式服务；本地演示和开发维护 profile 不适用本文件的线上限制 |
...
| 积分/额度不足 | 当前演示态 0 分仍可发送；这不能代表正式 token 计费 | 模型网关在调用前确认可用额度。付费模型余额不足返回 `insufficient_credits`，不请求供应商；显式标记为免费或套餐内额度的模型可按目录规则继续使用 |
...
| 请求关联 | 客户端每个用户动作生成 UUID `clientRequestId`；透传到网关、账本、模型日志和客服工单 |
...
| 策略完整性 | `catalog`、`capabilities` 与高风险权限策略置于 policy envelope：含 `policyVersion`、`issuedAt`、`expiresAt`、`minClientVersion`、`keyId`、JCS 规范化 payload 的签名；客户端仅接受内置受信任公钥验证通过且未过期的策略 |
...
| HTTP | code | 客户端动作 |
| --- | --- | --- |
| 400 | `invalid_phone` / `invalid_code` / `invalid_request` | 标记字段或展示本地已翻译的输入错误 |
| 401 | `unauthorized` / `token_expired` | 单飞刷新 token；刷新失败回登录页 |
| 403 | `activation_required` / `feature_not_entitled` / `model_not_allowed` | 显示资格说明，不重试 |
...
| 402 | `insufficient_credits` | 显示余额不足，不调用本地 fake provider 兜底 |
...
| 16 | `/legal/documents` | GET | 可匿名或是 | 返回已批准的协议/隐私告知文档元数据与当前版本；不允许客户端使用开发占位文本替代 |
...
必备错误码：`invalid_phone`、`code_not_sent`、`code_invalid`、`activation_required`、`activation_invalid`、`phone_mismatch`、`too_many_requests`。`phone_mismatch` 不回传完整旧手机号；客户端将其导向真实客服入口，不做本地换绑。
...
- `POST /v1/legal/acceptances`：仅在 Q11 为“需要”时启用，请求含 `documentType`、`version`、`clientRequestId`；服务端以账号、文档类型/版本、时间、来源版本和 request ID 记录，重复提交幂等。拒绝同意的访问限制由 Q11 明确，客户端不能在本地缓存伪造接受。
...
2. 每次用户发起的模型请求只有一个 `clientRequestId`，它贯穿流式调用、账本和客服定位。
...
客户端应先校验版本连续性、大小上限、词条长度、摘要，再写入临时文件并原子替换。`baseVersion` 不匹配、摘要错误、内容不合法时丢弃本次更新并保持最近有效缓存。离线或服务不可用时继续使用内置词、用户词和最近有效服务器缓存。
...
| 接口兼容与弃用策略 | 每次 breaking change 有语义版本、最小客户端版本、至少一个发布周期的弃用窗口、changelog 和 fixture 版本；不得只改 `message` 文案或静默改字段语义 |
| Sandbox | HTTPS base URL、隔离测试账号/激活码、可控短信验证码、至少两个模型与可观察 token usage |
| Mock / contract test | 可在客户端 CI 中跑的 mock 或 pact；覆盖正常、401、402、409、429、5xx、SSE 中断 |
...
| E2E-15 | 正式协议与版本确认 | 发布包不含占位文本；当前文案的类型/版本/摘要/生效时间与批准清单一致。Q11 要求确认时，接受/拒绝/重复提交和版本升级均按合同处理，记录可审计 |
...
| ID | 待确认项 | 推荐默认 | 影响 |
| --- | --- | --- | --- |
| Q1 | 激活授权 365 天后是否仍仅提示、不拦截 | v1 仅提示；续期/客服入口可见 | 登录、bootstrap、账户页、客服流程 |
| Q2 | `microCredits` 与人民币/套餐的换算、每个模型的 token 单价、最大输出上限 | 价格完全由服务端版本化；客户端只展示结果/可选预估 | 网关预留、账本、购买与用户告知 |
| Q3 | 余额为零时哪些模型可继续用 | 仅目录明确标记为免费或套餐内的模型可用 | 402 行为、目录、产品文案 |
| Q4 | 首发要隐藏但保留的菜单/能力清单，以及首批灰度对象 | `visible=false`、能力实现保留、按账号灰度开启 | 能力矩阵、UI、权限测试 |
| Q5 | 模型目录首发清单及隐私模式允许的模型 | 隐私模式默认不含云端模型；任何例外需在目录中显式标注 | 模型网关、隐私告知、模式选择器 |
| Q6 | 更新签名算法、密钥托管与发布 owner | Ed25519 manifest 签名 + 可轮换 keyId | 发布服务、原生更新器、事故回退 |
| Q7 | 客服工单系统与服务时段 | 先明确单一入口、SLA 与升级 owner | `phone_mismatch`、退款/丢盘/事故闭环 |
| Q8 | 便携数据跨 Windows 用户/机器时的会话语义 | 会话/产物可按本地文件迁移；凭证不可迁移，需重新登录 | credential store、客服、数据迁移说明 |
| Q9 | 账号删除、导出与各类服务端记录的保存期 | 先确定适用地区、法务 owner 和合法保留例外；客户端不对本地删文件作虚假承诺 | 隐私协议、删除 API、客服/审计 |
| Q10 | 首发的内容安全/滥用策略与申诉 owner | 必须先给最小机器码、用户处置、账务终态和运营值班路径，再决定精细模型策略 | 网关、产品文案、客服、账本与上线门 |
| Q11 | 协议/隐私告知是否要求显式同意；拒绝同意时允许哪些本地能力 | 产品/法务按目标地区和数据流确认；接口先支持版本展示，确认记录仅在批准要求后启用 | 登录/启动、文案、审计与客服 |
