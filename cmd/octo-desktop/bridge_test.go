...
// plus the window-token query the product gate keys the window identity off.
// Both strings are duplicated across the Go/JS boundary (desktopShellQuery /
// productgate.QueryWindowToken vs web/src/lib/stores.ts / web/src/lib/
// product.ts); renaming one side without the other silently downgrades the
// desktop shell to a plain-web client (the OS file dialog and native header
// quietly stop working) or locks the window out of the product gate. This test
// locks the Go side of that contract.
func TestShellURL(t *testing.T) {
	...
	tests := []struct {
		...
		{"no hash, no token", "", "", base + "/?shell=octo-desktop"},
		{"with hash", "settings", "", base + "/?shell=octo-desktop#settings"},
		{"nested route", "chat/abc123", "", base + "/?shell=octo-desktop#chat/abc123"},
		{"no hash, with token", "", tok, base + "/?shell=octo-desktop&window_token=deadbeef"},
		{"hash and token", "settings", tok, base + "/?shell=octo-desktop&window_token=deadbeef#settings"},
	}
	for _, tt := range tests {
		if got := shellURL(base, tt.hash, tt.token); got != tt.want {
			t.Errorf("shellURL(%q, %q, %q) = %q, want %q", base, tt.hash, tt.token, got, tt.want)
		}
	}

	// The marker must survive on every variant — it is the sole nativeShell signal.
	if got := shellURL(base, "x", tok); !strings.Contains(got, "shell=octo-desktop") {
		t.Fatalf("shellURL dropped the desktop marker: %q", got)
	}
	// An empty token must not leave a dangling window_token= query.
	if got := shellURL(base, "x", ""); strings.Contains(got, "window_token") {
		t.Fatalf("shellURL leaked an empty window_token query: %q", got)
	}
}
