...
> **状态**：`v0.15` —— **版本号与变更内容都看 [`§6 修订`](#6-修订) 的顶行**，本节不复述（本行此前停在第 `v0.10` 而修订表已到 `v0.12`：同一个版本写两处，第二处必然落后 —— `V-56` 同型，2026-09-14 收敛）。
...
`v1` 上**后端的落地是部分的**（2026-09-12 更正，原文说"还没有真后端"已过期）：[`internal/productruntime`](../../../internal/productruntime) 已随 `PR-2b1` / `PR-2b2a` 落地并接进服务，**§1.4 的 15 条端点里有 14 条**（`state` / `send-code` / `login` / `logout` / `locale` / `nickname` / `prefs` / `chat-modes` / `sensitive/dict` 的 `GET`＋`PUT` / `sensitive/dict/import` / `control-plane` / `catalog` / `credits`）**是真实实现、可反向核对**；其余 1 条（`POST /api/product/sensitive/check`）仍是正向整理 —— **它今天还没注册**，见 §1.4 的 ⬜ 标记、`V-24` 与 `PR-6b3`。所以本文件的可信度是**混合**的：
...
**标 `✅` 不等于「后端已验证」**（2026-09-12 收紧）。上一句只保证"前端依赖它"。要算**后端已验证**，还须该行落在上表第一行的十四条已实现端点里。**2026-09-14 更正（`V-24` 的相邻面）**：此前这条口径与 §1.4 的 ✅ 列合起来仍会误导 —— 四条词库/检测路由在 §1.4 标着 `✅`、在这里又被列进"其实没有后端"，读者分不出哪一列在说形状、哪一列在说存在（**2026-09-14 晚：其中三条已由 `PR-6b2` 挂上，只剩 `sensitive/check`**）。**现在 §1.4 的状态列直接标存在性**（⬜ ＝ 该路由今天没有注册），两处不再互相打架。
...
| 12 | POST | `/api/product/sensitive/check` | 否 | 否 | ⬜ **未注册**（`V-24`；§2.12 形状已定，归 `PR-6b3`） |
...
> **状态列说的是「这条路由今天在不在」**（2026-09-14 明确，`V-24`）。判据是可核对的一件事：它在 `internal/productruntime` 的 `Mount` 表里有没有一行 `api(...)`。**在此之前这一列只表达了"形状定了没有"**，于是四条词库/检测路由挂着 `✅` 而代码里一条都没有（2026-09-14 晚 `PR-6b2` 挂上三条，剩 `sensitive/check`） —— 这正是 `V-24`（前端 `sensitive.ts` / `sensitiveDict.ts` 调得很顺，真机一律 404）能在文档里长期不可见的原因：**"形状已定"和"路由已挂"被同一个记号表达了**。`🚧` 仍表示"形状本身还在改"（只有第 3 行）。
