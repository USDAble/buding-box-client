...
flowchart TB
    ...
    RUNTIME --> PG[internal/productclient/gateway<br/>observer contract + terminal control]
    ...
    DESKTOP --> APP[internal/app<br/>single provider-client constructor]
...
**注意 `DESKTOP --> APP --> PROV` 这条链**：中台网关的模型流不是 `productclient/gateway` 自己实现的 HTTP/SSE，而是复用既有 provider 栈（`app.ProviderCustom` 已支持任意 BaseURL / headers / 限流）。**装配只能发生在宿主 `cmd/octo-desktop`** —— `gateway` 与 `productruntime` 都不得 import `internal/app`（`internal/productruntime/deps_test.go` 的 `bannedImports` 会红），`gateway` 只声明观察 contract（`Observer.Wrap` / `CallMeta` / `TerminalSink`），宿主用 `app.NewSender` 造好 base 后交给 `productruntime.Observe(base, meta, sink)`。这样上游对 SSE / tool call / usage 的修复会自动惠及中台链路。
...
| `internal/productclient/gateway` | 声明网关调用的**观察与终态控制 contract**：`Observer.Wrap(base, meta, sink)`、`CallMeta`、`TerminalSink`、`ControlClient`（取消/状态），并把观察结果转成会话事件/终态 usage | **装配**（它不得 import `internal/app`）、供应商 key、价格计算、账本写入、工具授权、**自己的 SSE/HTTP/重试实现（复用 `internal/provider`）** |
| `internal/productpolicy` | 能力矩阵与策略判定：`audience` / 时间窗口 / 版本单调 / 过期语义，给本地 PEP 返回 `allow`/`ask`/`deny`；消费当前 Profile 和受信动态策略。**验签与 `data/catalog.json` 的原子写/刷新调度不在本包** —— 五段归属见 [P0-04 §归属定案](04-模型目录能力矩阵与策略执行.md#归属定案五段各一个唯一-owner) | 远程 HTTP、`data/` 的落盘与刷新调度、UI 显示、替代 `internal/permission` 的上游机制 |
