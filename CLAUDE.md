...
2. **Never hardcode a brand string.** All product names interpolate `{brand}` / `{brandShort}` from `branding/brand.json`; `scripts/brand-guard.mjs` enforces it (third leg of `make brand-check`, CI job `brand-guard`). It fails a product file that types the copy in, and fails either user-facing copy table (`web/src/lib/i18n.ts`, `cmd/octo-desktop/lang.go`) that names the upstream "Octo". Two strings it deliberately does **not** treat as brand copy — no annotation needed for either: the literal "Octo" in the port-conflict message (that names the *upstream* product, not ours), and the ASCII identifiers `BUDING-DEMO-0001` / `buding-*` model ids (fixed data keys — they do **not** follow the English brand name).
