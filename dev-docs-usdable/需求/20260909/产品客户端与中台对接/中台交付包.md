...
**客户端本地产生的 fail-closed 码**（不在中台响应里，但同样登记在本节，前端映射表必须包含）：`catalog_signature_invalid`、`policy_expired`、`bootstrap_stale`、`usage_pending`。前三个是本地验签/版本/时钟校验失败，保留当前会话展示但阻断新 Agent 与新模型请求；`usage_pending` 表示流中断后结算未确定，只能查询 `RequestStatus`，不得自行回补或扣减。
