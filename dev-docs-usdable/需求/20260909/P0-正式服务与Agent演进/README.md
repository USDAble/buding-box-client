...
2. **这不构成 §4.1 的豁免**：本批次**仍然满足**「缺「上游合并影响」一节视为未完成」的实质要求，只是那一节的位置在 01A 而不是每份文件里。**新增工作包时，如果它触碰上游文件，必须在 01A 登记该文件与预算** —— 登记即视为该工作包的「上游合并影响」已写。若某个新工作包触碰上游文件却**没有**在 01A 登记，视同缺节，按 §4.1 处理。
