...
2. **Never hardcode a brand string.** All product names interpolate `{brand}` / `{brandShort}` from `branding/brand.json`; `scripts/brand-guard.mjs` enforces it (third leg of `make brand-check`, CI job `brand-guard`). It fails a product file that types the copy in, and fails either user-facing copy table (`web/src/lib/i18n.ts`, `cmd/octo-desktop/lang.go`) that names the upstream "Octo". Two strings it deliberately does **not** treat as brand copy: the literal "Octo" in the port-conflict message (that names the *upstream* product, not ours), and the ASCII identifiers `BUDING-DEMO-0001` / `buding-*` model ids (fixed data keys — they do **not** follow the English brand name). Only the first needs anything from you, and what it needs is a licence rather than nothing: the port-conflict copy must carry a `// brand-exception: <why>` marker on the comment line directly *above* the string. 需求20260906 §5.1.2 第 6 条 mandates that sentence naming the upstream product, because the situation it describes is the user having upstream Octo installed — interpolating our brand there would tell them to quit the wrong program. `brand-guard` leg 2 and `TestNativeStringsCarryNoBrandLiteral` both read that marker, so one annotation covers both, and a marker left on copy that no longer names it is itself a failure.
...
- **Branch off latest `v1`** before editing. Feature PRs target `v1` (the product integration branch); `main` is the upstream-tracking branch. Never commit on either directly. One feature = one PR = one squash commit.
