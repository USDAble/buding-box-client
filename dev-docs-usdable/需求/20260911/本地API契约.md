...
| 事件 | 载荷 | 接收方 | 说明 |
| --- | --- | --- | --- |
| `credits_update` | **无（纯触发）** | `App.svelte`（全局 ⇒ 每个窗口） | ✅ **发射方已落地（`PR-8`，2026-09-16）**：触发＝**一回合结束**（`ws_handlers.go` 的 turn tail，与全局 `session_activity` 伴播同处），**全局广播** —— 余额是全局事实，而 `complete` 只到订阅那个会话的窗口。**载荷刻意为空**：前端 `App.svelte:273-284` 只把它当催促、数字一律来自 `refreshCredits()`（`E9` 规则 2）⇒ 带 `credits` 会让它变成账本那个数字的第二个写入者（陈旧/乱序的推送会覆盖更新的读取）。**不重放**（冷启动本来就从账本拉） |
...
| `datastore:lost` | 无 | `App.svelte`（全局） | 数据目录失联（U 盘拔出）→ 冻结遮罩，阻止所有输入。**触发**＝壳侧 watchdog 的**丢失转换**（`cmd/octo-desktop/watchdog.go`；2 次连续失败才翻，且一次丢失只广播一次 —— 迟滞与去重都在那一处，本事件不做第二次判定）。**新连接必须重放**：连上时若 `datapath.Frozen()` 为真就补发一次 —— 否则**冻结期间刷新或重开窗口**会让 `frozen` 复位、遮罩消失、输入框复活，而写入**仍然被拒**（那正是这条闭环 `L-E3` 的判据）。**范围（§3.10）**：只在便携桌面壳里有产生方（watchdog 属于 `cmd/octo-desktop`），`octo serve` 不发射 |
| `datastore:restored` | 无 | `App.svelte`（全局） | 同路径恢复 → 解冻。**触发**＝同一 watchdog 的**恢复转换**（只认**监视的那个绝对路径**重新可用；换盘符不算恢复，同路径换了一块盘分不出 —— 见 `V-88`）。**不重放**：未冻结是新连接本来的状态，没有需要补的真相 |
...
> **四事件的作用域／触发／重放已定（2026-09-16，`G1`＝`N-5`）**，逐条见上表。三条通则：**① 作用域**——`session_id` 缺失即**全局**，这是消费点位置决定的（三个消费点都在 `App.svelte`，会话之外），不是设计选择；`input_sensitive` 是**唯一**的会话级，因为它属于某一个会话的输入框。**② 触发**各自只有一个 owner（turn tail / 输入门 / 壳侧 watchdog），本文件不复述其内部判定。**③ 重放**只有 `datastore:lost` 需要，理由是"窗口可能在冻结**期间**才连上"。`credits_update` 与 `datastore:restored` 都不重放。**派生面**：契约的事实来源是**代码**（`internal/server/product_events.go` ＋ 三处发射点），本表只登记形状与语义。
...
| `N-5` | §4 四个 WS 事件的作用域与重放语义 | ✅ **已定（2026-09-16 人工，`PR-8`）** —— 作用域＝**全局**（`input_sensitive` 是唯一会话级）、触发与**重放**逐条见 §4 的表（其中"`datastore:lost` **必须在新连接重放**"是 `L-E3` 的判据）。**本表因此不再有"真正未定"的条目** —— 剩下的 `S-1` 属「**已定规格、未实现**」，不是未定 |
