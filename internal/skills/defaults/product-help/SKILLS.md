...
Same format as Claude Code's — `~/.claude/skills` can be symlinked to `<data root>/skills` to reuse skills you already have.
...
`name` and `description` are the only required frontmatter fields — the description is what the model sees to decide when the skill applies, so make it specific about triggers. Drop it under `<data root>/skills/<name>/`. The `skill-creator` default skill can also build one interactively.
