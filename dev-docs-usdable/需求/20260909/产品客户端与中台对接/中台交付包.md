...
中台不是为客户端提供一组“替换 旧本地实现 的 CRUD 接口”，而是正式产品的账号、模型可用性、用量账本、通用词库、能力开关和更新清单的权威来源。客户端保存会话和用户本地设置，中台不保存供应商 API key 到客户端，也不把计费计算交给客户端。
...
| 402 | `insufficient_credits` | 显示余额不足，不调用本地 合同测试实现 provider 兜底 |
...
- OpenAPI 3.1、错误码注册表、测试环境 URL、测试账号与可重复的 旧本地实现 server 是中台交付物的一部分，不是联调时临时口头约定。
...
| Q9 | 账号删除、导出与各类服务端记录的保存期 | 先确定适用地区、法务 owner 和合法保留例外；客户端不对本地删文件作虚旧承诺 | 隐私协议、删除 API、客服/审计 |
