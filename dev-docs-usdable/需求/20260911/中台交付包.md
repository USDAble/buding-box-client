...
| 产品基线 | 产品范围、用户行为和不可突破的安全/计费约束以 [P0 正式上线需求基线](需求基线.md) 为准；若接口合同与该基线冲突，先更新基线并完成三方确认 |
...
sequenceDiagram
    ...
    D->>G: POST /v1/chat/completions（model=目录 id、stream）
...
> **`phoneMasked`（2026-09-11 补，`PR-2b` 实施时发现缺字段）。** 仅 `phone_mismatch` 带，值为**激活码已绑定的那个手机号（脱敏）**。为什么必须由中台给：触发这条的典型场景是「**全新 `data/`** + 一个已被绑定的激活码」——此时客户端**从没见过**那个号码，本地状态里没有可回显的值，而界面又必须能说出"此目录已绑定 138\*\*\*\*1234"（`本地API契约.md` §2.3）。客户端侧的对应实现是 `internal/productclient` 的 `Error.PhoneMasked` → `productruntime` 透传。
>
> **`field` 的效力范围（同日明确）。** 中台可以在错误体里带 `field`，但它只表达"中台认为该错误与哪个字段相关"，**不决定客户端把错误渲染到输入框还是顶部横幅** —— 那个层级由客户端契约（`本地API契约.md` §3）决定。例：中台把 `activation_invalid` 标成 `field: "activationCode"`，而客户端把它渲染为**业务级横幅**，因为用户改不动这个值。中台不得假设客户端会按 `field` 落位。

**客户端本地产生的 fail-closed 码**（不在中台响应里，但同样登记在本节，前端映射表必须包含）：`catalog_signature_invalid`、`policy_expired`、`bootstrap_stale`、`usage_pending`、`network_unavailable`、`model_withdrawn`、`input_sensitive`。前三个是本地验签/版本/时钟校验失败，保留当前会话展示但阻断新 Agent 与新模型请求；`usage_pending` 表示流中断后结算未确定，只能查询 `RequestStatus`，不得自行回补或扣减；`network_unavailable` 表示请求根本没有到达中台（与 `upstream_unavailable` 区分：后者是中台回了 5xx），它同样**不**构成回落到本地模型或 `config.yml` 的理由。

> **`input_sensitive`（2026-09-15 补，`PR-6b3`）。** 用户手敲的正文命中本地词表 ⇒ 服务端在**回合入口**拒绝该回合（不广播、不落库、不建会话、不发一个字节），并把**已打码**的文本交回前端。与 `model_withdrawn` 同族：本地产生的拒绝必须带码，否则前端只能按文案认它。**形态与两者不同**：`model_withdrawn` 走 `turn_error` 的 `code` 字段（回合已存在），本条走 `400 {"code": "input_sensitive", "text": …}`（回合尚不存在，用户要看的是"往框里放什么"）与 `/ws` 的 `input_sensitive` 事件 —— 两个载体的形状都登记在 `本地API契约` §3 / §4。**它不是中台的码，也不得当作中台拒绝的回执**。

> **`model_withdrawn`（2026-09-14 补，`PR-5d3`）。** 会话绑定的模型已不在本构建拿到的目录里 ⇒ 客户端在**建立 sender 之前**拒绝该回合（不新建 Agent、不发一个字节），并把这个码发给前端。**为什么必须是一个码而不是一句话**：这条拒绝是本地产生的英文句子，而 `B8` 规则 2 要求界面说中文；没有码时前端只能按**文案**去认它 —— 改一次措辞就静默失配（`V-55` 就是这么活了很久）。它同时是 `turn_error` 携带 `code` 的两个消费点之一（另一个是 `insufficient_credits`），而 `C8` 规则 2 要求「所有码都有 UI」的**完整**码表仍未做，登记为 `V-67`。

| HTTP | code | 客户端动作 |
| --- | --- | --- |
| 400 | `invalid_phone` / `invalid_code` / `code_not_sent` / `activation_invalid` / `activation_code_used` / `box_code_unknown` / `box_code_mismatch` / `invalid_request` | 标记字段或展示本地已翻译的输入错误。**激活三码必须分别渲染**（激活码无效 / 激活码已用过 / 盒子编号不存在 / 与激活码对不上），且都要给出客服入口 —— 否则用户抄错一位就永久卡死（`需求基线` `E1` 规则 2、`PQ9`） |
| 401 | `unauthorized` / `token_expired` | 单飞刷新 token；刷新失败回登录页 |
| 402 | `insufficient_credits` | 显示余额不足，不调用本地 mock provider 兜底。**客户端不得在本地预测这个码**（`PQ8`：免费/套餐额度下余额为 0 仍可能可发），充值入口常驻、不随余额出现或消失；文案由前端按 `code` 取键（`本地API契约` §3），**不渲染 `message`** |
| 403 | `activation_required` / `plan_expired` / `feature_not_entitled` / `model_not_allowed` | 四种状态阻断动作互不相同（§1.2）：未激活→激活/客服；套餐到期→续费；能力未授权→说明；模型无资格→刷新目录后重选。是否阻断聊天按 §1.2 产品确认，但**码值先登记**，前端映射表不得缺项。**`activation_required` 的三个触发时机与形状（2026-09-13 补，`V-45`）**：① **登录**且该手机号**没有可用授权** —— 包括"从未注册"与"注册了但这份授权未激活"两种情况，**中台不必区分**（`§4.2` 里那条"不得泄漏号码是否已经注册"针对的是 **`send-code`** —— 登录发生在验码之后，用户已证明自己持有该号码，因此不构成枚举泄漏；作用域见 `开发规范` §3.10）；② 已登录会话中该账号被撤销/到期后发起受保护请求；③ 它**一律业务级、不带 `field`**（问题在账号而不在某个输入字段）。**替身此前在这个时机错发 `activation_invalid`（400）**，客户端于是把"该去激活"渲染成"激活码不正确"、落在**没有该字段的表单**上 —— 替身已按本行改正（`clienttest/server.go`） |
...
`activationCode` 与 `boxCode` **仅首次激活必填**，两者**独立校验、都必须有效**，后续登录省略。语义区别：`activationCode` 回答「这份授权有没有买」，`boxCode` 回答「这份授权对应哪个盒子」；**一个 `boxCode` 可搭配多个 `activationCode`（一对多）**，但**一个 `activationCode` 只能使用一次**（需求基线 `E1` 规则 2，2026-09-11）。`boxCode` 是业务提供的一个字符串，**不由任何硬件标识派生**，中台按明文存储与校验。
...
```json
{
  "data": {
    ...
    "activation": { "status": "active", "activatedAt": "2026-09-10T08:00:00Z", "expiresAt": "2027-09-10T08:00:00Z", "boxCode": "BOX-XXXXXX" }
  },
  "requestId": "uuid"
}
```

必备错误码（**取值以 §3.2 登记表为准，此处只列登录会遇到的哪些**）：`invalid_phone`、`code_not_sent`、`invalid_code`、`activation_required`、`activation_invalid`、`activation_code_used`、`box_code_unknown`、`box_code_mismatch`、`phone_mismatch`、`rate_limited`。`phone_mismatch` 不回传完整旧手机号；客户端将其导向真实客服入口，不做本地换绑。**激活类失败必须逐个区分**：`activation_invalid`（激活码无效）、`activation_code_used`（激活码已用过，一次性）、`box_code_unknown`（盒子编号不存在）、`box_code_mismatch`（两者对不上）—— 客户端为每个码渲染专属文案并给出客服入口。（2026-09-11 更正：原文写的 `code_invalid` / `too_many_requests` 是 §3.2 之外的第二个命名，已按 §3.2 收敛为 `invalid_code` / `rate_limited`。）
...
```json
{
  "data": {
    ...
    "activation": { "status": "expired", "expiresAt": "2027-09-10T08:00:00Z", "boxCode": "BOX-XXXXXX" },
    ...
    "policySignature": { "keyId": "policy-2026-a", "sig": "base64-ed25519-signature-over-the-exact-bytes-of-data.policy" },
...
**签名对象（2026-09-11 拍板，取代 JCS 方案）。** 签名**不**放在 `policy` 内，而是同级字段 `policySignature`；被签名的数据是 **`data.policy` 值的原始 JSON 字节**（服务端先序列化 `policy` 一次，对这段字节签名，再把同一段字节原样嵌入响应）。客户端对**收到的原始字节**验签，不做任何规范化。

**为什么不做 JCS 规范化（RFC 8785）而签原始字节。** 两条路都能达到同一安全目标；本轮取简单的一条，正规的一条登记为 [`待解决问题.md`](待解决问题.md) `D-007`：

- **签原始字节**：验签方与签名方对"被签名字节"**不可能产生分歧**（两边签的就是同一段字节）。代价：任何会**重新序列化 JSON** 的中间人（某些代理、网关）都会破坏签名 —— 本产品的客户端**直连中台**、无中间人，故无实际影响。
- **JCS 规范化**：对中间人鲁棒（重新序列化无害），但要求双方各自实现 RFC 8785，其中最危险的是数字规范化（要求 ECMAScript `Number::toString` 的最短往返表示）。**两份实现会漂移**，届时表现为"替身测试全绿、生产验签失败"。本轮不做。

**若将来切回 JCS**：`policySignature` 的形状不变，变的只是"被签名的字节"从原始字节改为 `JCS(policy)`；并且 `signature` **仍然**留在 `policy` 外 —— 在 `policy` 内放签名字段会迫使验签方"反序列化→删除字段→重新序列化"，那正是规范化分歧的入口。

**`keyId` 出现两处的说明：** `policy.keyId` 是**被签名**的一份（供客户端读取并比对缓存元数据），`policySignature.keyId` 是**签名元数据**（供验签时选择公钥，可在 `policy` 解析失败时先行使用）。两者必须一致；不一致按验签失败处理。**信任锚**是编译期 `productprofile.TrustedKeyIDs`（keyId → base64 ed25519 公钥），客户端**不接受**响应里携带的任何公钥。

**策略版本只有一个位置：`policy.policyVersion`（签名覆盖范围内）。** 外层 `data` **不得**再出现第二份 `policyVersion`。理由不是"避免歧义"而是安全：签名覆盖范围之外的字段不参与验签，中间人可以改写客户端读到的版本号而签名仍然验签通过 —— 那会让"版本单调"与"策略撤销"两道防线同时失效。客户端读取策略版本时只允许取自验签通过的 `policy`。
...
`GET /v1/catalog/models` 返回**与 `bootstrap` 同一个信封形状**（`{policy, policySignature}`），用于客户端主动刷新。

> **为什么不是"只返回 `policy.catalog`"（2026-09-13 改）：** 签名覆盖的是 `data.policy` 的**原始字节**，单取 `catalog` 就把它拆散了，客户端拿到手**无法验签** —— 等于给了一份"必须相信"的目录。同一事实不能有两种形状（`开发规范` §3.8），所以刷新与 `bootstrap` 同形。客户端因此**没有**任何"刷新的目录不验签"的分支。

条件请求：客户端把**已有的** `catalogVersion` 作为 `knownVersion` 查询参数带上（不引入 `ETag` —— 它要在客户端缓存里多存一项不透明元数据，而 `knownVersion` 复用已有字段）。未变化时中台**二选一**：

| 应答 | 说明 |
| --- | --- |
| `304 Not Modified` | 无响应体 |
| `200` + `{"data": {"unchanged": true}}` | 无 `policy` 字段 |

**客户端两种都必须认**（`中台交付包` 允许中台任选，客户端不得只认一种）；收到任何一种都**不得重写本地缓存**。

> **中台的义务（重要）：** 对**已过 TTL** 的目录，中台**不得**回 `304` / `{"unchanged": true}`。`unchanged` 的含义是"你手上那份仍是当前版本"，而它的签名与新鲜度声明都没有变，所以它**仍然过期** —— 客户端会（正确地）保持"需要联网更新"并拒绝开启新回合。TTL 到期后中台必须下发一份**新的**签名目录（新 `version`，或至少新的 `issuedAt`）。这一条不做的话，客户端会在目录过期后被永久卡住，而它没有任何本地办法自救。
...
#### `catalog.modes` 的形状（2026-09-11 定，只放"模型里没有的那部分"）

**分组事实的唯一 owner 是模型对象的 `modeIds`** —— 一个模型属于哪些模式，写在模型上。因此 `catalog.modes[]` **不得**再列 `modelIds` / `models`：那是同一个事实的第二处定义（[开发规范](../../开发规范.md) §3.8），两者一旦不同步，选择器会在"按模型投影"与"按模式投影"两条路径上给出不同结果。

`modes[]` 只承载**模型侧无法表达**的模式级元数据：

```json
"modes": [
  { "id": "privacy", "defaultModelId": "buding-privacy-1" },
  { "id": "smart",   "defaultModelId": "buding-cloud-pro" },
  { "id": "default", "defaultModelId": "buding-cloud-pro" }
]
```

| 字段 | 说明 |
| --- | --- |
| `id` | **产品常量**（`privacy` / `smart` / `default`），不是中台可自由命名的值。出现未知 id ⇒ 客户端忽略该条并记录，**不得**据此新建模式（模式集合由产品定，不由目录定） |
| `defaultModelId` | 该模式下的默认模型；必须是同一目录里 `eligible=true` 且 `transport=gateway` 且 `modeIds` 含该模式的模型，否则该条视为无效 |
| ~~`modelIds` / `models`~~ | **禁止**。分组从 `models[].modeIds` 推导 |
| ~~`displayName`~~ | **禁止**。**模式展示名是界面文案，不是目录数据** —— 三个模式的展示名固定在客户端 i18n（`mode.privacy` / `mode.smart` / `mode.default`），按 [需求基线](需求基线.md) `B5` 规则 1「展示名是界面文案、分组归属来自目录，两者不得互相推导」。本节早前由 [`本地API契约.md`](本地API契约.md) §2.8 承担的那处 `modes[].displayName` 属界面投影，不是中台下发内容 |
...
`displayName` 的形状是**中英双语对象** `{"zh": "...", "en": "..."}`（2026-09-11 拍板，与本文件早前示例的纯字符串形状冲突，以本行为准）。**客户端按当前界面语言取值、缺键回落 `en`、再缺回落 `id`** 的完整规则登记在 [`需求基线.md`](需求基线.md) `B6` 规则 2，本节不复述。
...
| 版本可比 | `version` 必须匹配 `YYYY-MM-DD` 或 `YYYY-MM-DD.<计数器>`，且**同一天内的计数器按数值递增**。客户端**按各段数值比较**，因此**不要求定宽零填充**；但形状不匹配时客户端只能退回字符串比较（见 `internal/catalogstore` 的 `compareCatalogVersions`），那**不保证**是版本次序。例：`2026-09-09.9` → `2026-09-09.10` 合法且必须被识别为更新（字符串序会判成回滚，这正是客户端不按字符串比的原因） |
| 无凭证依赖 | 目录信封本身不得包含任何供应商连接信息，也不得把"是否可缓存"设计成需要 token 才能解密（离线缓存需要可独立验签） |

**客户端侧落地**（见 [P0-04](需求基线.md) §模型目录的本地缓存与刷新）：`data/catalog.json` 保存原始签名信封 + `catalogVersion`/`fetchedAt`/`expiresAt`/`keyId`/`audience`，原子替换写入；启动刷新、按 TTL 周期刷新（登记为 `TODO`，见 `开发计划` §2 `PR-4c`）、陈旧时按需刷新、换账号时强制刷新。**过期且刷新失败时不得开启新回合；验签失败时 fail-closed；无缓存时不得回落到本地模型或 `config.yml`。** 四种可用性状态（`ready` / `absent` / `stale` / `unverifiable`）由 `GET /api/product/catalog` 上报，`本地API契约` §2.14。

### 4.4 能力 ID 注册表（初稿；Q4 确认后冻结）

> **2026-09-15 —— 本表在客户端没有消费者（重要，避免中台按本表下发却发现不生效）。** 客户端**永久不实现**能力矩阵（`需求基线` `B7`、`待解决问题.md` `D-009`，人工拍板）。因此：**下发表中的 `visible` / `entitled` / `permissionPolicy` 不会改变客户端任何行为** —— 入口可见性由客户端的静态常量决定（`web/src/lib/features.ts`：藏 `channels` / `mcp` / `lightapps`，露 `skills` / `workflows` / `tasks` 等），本地权限门沿用上游默认（默认 `ask`，动手前必问）。**下表保留为「服务端若要表达这些概念时的命名登记」**（`id` 与字段名的唯一来源），但**不要再把它当作客户端的接口契约**。`Q4` 因此不再是客户端必答项。

> **这是 `capabilities[].id` 的唯一登记表。** `available` / `visible` / `entitled` / `permissionPolicy`、以及 P0-11 的入口可见性都挂在同一个 id 上；任何实现都不得再造别名（开发规范 §3.8）。（**2026-09-15 更正**：原文此处还写"客户端 `productpolicy` 的常量表"——客户端**不做**，见本节抬头。）中台的账号/套餐/灰度配置若要使用这些 id，必须用同一组，否则「菜单隐藏」与「执行拒绝」会指向不同的东西 —— 但**今天客户端不读它们**。

本表是 [P0-00A](需求基线.md) 承诺的「首发起始入口清单 + capability ID 初稿」，按当前仓库**已存在的入口与执行面**列出。**Q4 只确认这份清单的增删与首发 `visible`/`entitled` 取值，不重新命名** —— 改名会让已下发的策略失配。`permissionPolicy` 的默认列是 `ask`，不是 `allow`；`allow` 只用于用户当场可见、无外部副作用的读动作。

> **2026-09-15：`Q4` 对客户端不再是必答项。** 客户端不消费本表，故本表的**首发 `visible`/`entitled` 取值对客户端无影响**（下发不生效，见本节抬头）。中台若要为**未来可能重开**的能力矩阵冻结命名，仍可确认本表；但**不阻塞任何客户端实现**。
...
> **路径更正（2026-09-13）。** 本节原写 `POST /v1/ai/chat/completions`，**该路径不可达**：客户端复用的是既有 OpenAI provider，其路径是**编译期常量** `/v1/chat/completions`（`internal/provider/openai/client.go` 的 `ChatCompletionsPath`），客户端侧没有任何可配置项能把它变成 `/v1/ai/…`。若要保留 `/v1/ai/…`，客户端就必须**改上游常量**（带 `// OCTO-FORK:` 标记），与 `C2`「优先不改上游」相抵。故**契约侧改为标准路径**，`model` 传目录 `id`。这是 `V-37` 证伪后**仍然成立的那一半**（`V-37` 的另一半"`GatewayHost` 含 `/v1` 会造成双前缀"已撤回 —— 见 `需求基线` §5.6 `V-37` 行）。
...
**标准 OpenAI 字段原样透传，本文件不重复声明。** 客户端复用 `internal/app` 的 provider 栈（[P0-01 §gateway 的复用形态](../../运行时Profile配置.md)），因此 `tools`、`tool_choice`、以及 `messages` 中 `role:"tool"` 的 `tool_call_id` 结果消息等**多轮工具调用**字段，都按 OpenAI `chat/completions` 语义发出；网关必须原样接受并转给供应商，不得拒绝或改写这些字段。同理，响应侧的 `tool_calls` 分片拼接由客户端 provider 完成 —— 所以 §5.3 的 `tool_call` 事件描述的是**模型请求工具**，而不是"我们还需要定义工具结果如何回传"。

> **本节要求自 2026-09-13 起是可判的（`PR-5b2`）：** 替身已经会发工具调用，且**故意把 `function.arguments` 按 `tool_calls[i].index` 切成 2–3 片**（`clienttest` 的 `splitInThree`）—— 单片会让客户端那条按 index 归并的路径**永远失练**，而它是本节的实质要求所在。中台按本节实现时，分片与否都可以，但**必须带 `index`**：客户端是按 index 归并的，不是按到达顺序。证据与手工走查见 `E2E闭环清单.md` `L-C3c` 与 `开发计划.md` §`PR-5b2`。
...
**推理档位：`reasoning_effort` 必须被接受，且不得因不认识的字段 400（2026-09-13 新增要求）。**

客户端把用户的推理档位按 OpenAI 标准形状发在**扁平的顶层 `reasoning_effort`**（取值 `low`／`medium`／`high`／`xhigh`／`max`），网关必须**原样转给供应商**，不得拒绝、不得改写、不得忽略成静默失败（忽略了的表现是"用户选了档位但思考块永不出现"，客户端无法分辨这与"该模型本就不推理"）。两条边界：

- **档位为"关闭"时客户端完全不发该字段**（`omitempty`），因此"请求里没有 `reasoning_effort`"是合法且常见形态，网关必须同样接受；
- **网关不得因为请求里出现自己暂时不认识的字段而返回 400。** 客户端复用的是通用 OpenAI provider 栈，会发出未来版本的字段（例如 `stream_options`、`tool_choice`）；把"未知字段一律拒绝"写进网关，等于把客户端**每次**字段增减都变成一次停机不兼容 —— 与本节上面"标准 OpenAI 字段原样透传"同一条原则（`PQ27`）。

> **为什么必须写死形状**：客户端 provider 用 `Dialect` 区分五种推理字段形状（扁平 `reasoning_effort`／嵌套 `reasoning:{effort}`／`enable_thinking` 布尔／`thinking:{type}`，见 `internal/provider/openai/client.go`），网关属哪种**在写本文件时无人知道**。本契约取**标准 OpenAI 扁平形状**；若中台实际形状不同，客户端改动点只有 `GatewayEndpoint.Sender()` 一处（不散在请求结构里）—— 但**必须先说**，否则 `L-C3b` 在真机上不可判。
...
| E2E-11 | 复制介质与 token 撤销 | 复制 `data/` 到另一 Windows 用户/机器**可复用登录态**（便携 U 盘的产品定位，2026-09-11 变更，见 §11）；但**撤销/轮换 refresh token 后旧副本必须重新登录**，诊断/导出无凭证 |
...
| Q4 | ~~首发要隐藏但保留的菜单/能力清单，以及首批灰度对象~~（**清单与 id 见 §4.4**）**—— 2026-09-15：客户端永久不实现能力矩阵，本项对客户端不再是必答项**（隐藏清单已由客户端静态常量承担，见 `D-009`） | ~~`visible=false`、能力实现保留、按账号灰度开启~~ **客户端不消费**：今日隐藏 = `web/src/lib/features.ts` 的 `HIDDEN_VIEWS`（藏 `channels`/`mcp`/`lightapps`） | 能力矩阵、UI、权限测试 | 产品 | ~~P0-04、P0-11~~ **无（不阻塞客户端）** |
...
| Q8 | 便携数据跨 Windows 用户/机器时的会话语义 | 会话/产物按本地文件迁移；**凭证也随 `data/` 迁移、可复用登录态**（2026-09-11 由产品拍板，替换原「凭证不可迁移」，见 §11） | credential store、客服、数据迁移说明 | 产品 / 安全 | P0-08、需求基线 `E6` |
...
| 日期 | 说明 |
| --- | --- |
| 2026-09-15 | **§4.4 加抬头「本表在客户端没有消费者」＋ `Q4` 标为"不再是客户端必答项"。** 客户端**永久不实现**能力矩阵（`需求基线` `B7`、`待解决问题.md` `D-009`，人工 2026-09-15 拍板），因此中台若按本表下发 `visible` / `entitled` / `permissionPolicy`，**不会改变客户端任何行为** —— 这条必须写在中台能看见的地方，否则会出现"中台按合同下发了、客户端静默不生效"这一类最难定位的问题（契约上没错，只是没人读）。同时删掉 §4.4 抬头里"客户端 `productpolicy` 的常量表"那半句（客户端不消费 id，故不需要常量表）。**本表保留**为服务端表达这些概念时的**命名登记**；`Q4` 若要冻结命名仍可确认，但不阻塞任何客户端实现。**范围（§3.10）**：中台 ↔ 客户端的能力矩阵契约面；不含已实现的「模型可选资格」（那半在 `目录` 里，见 `需求基线` `B7`） |
| 2026-09-15 | **§3.2 的「客户端本地产生的 fail-closed 码」补 `input_sensitive`（`PR-6b3` 落地）。** 它是本地词表判定的拒绝：用户手敲的正文命中 ⇒ 服务端在**回合入口**拒绝（不广播、不落库、不建会话、不发一个字节），把**已打码**的文本交回前端。**与 `model_withdrawn` 同族**（本地产生的拒绝必须带码，否则前端只能按文案认它），**但载体不同**：后者走 `turn_error` 的 `code`（回合已存在），前者走 `400 {"code": …, "text": …}`（回合尚不存在 —— 用户要看的是"往框里放什么"）与 `/ws` 的 `input_sensitive` 事件。**它不来自中台，也不得被当作中台拒绝的回执。** 两个载体的形状登记在 `本地API契约` §3 / §4（本条只登记码值本身）。**范围（§3.10）**：客户端本地码表；不新增中台接口、不改任何中台应答。 |
| 2026-09-13 | **§5.2 补一条"本节已可判"的注解（`PR-5b2`），要求本身未改。** 工具调用的透传要求（`tools` / `tool_choice` / `role:"tool"` + `tool_call_id`、响应侧 `tool_calls` 分片由客户端归并）写在这里已有一段时间，但此前**没有任何东西能判它**：替身不会发工具调用（`clienttest` 只发 `content` 与 `reasoning_content`）。`PR-5b2` 让替身会发，并**故意分片**（`splitInThree`，2–3 片），把"按 `index` 归并"这条路径变成可端到端判的。故**新增的只是注解**：① 中台按本节实现时分片与否都可以，但**必须带 `index`**（客户端按 index 归并，不按到达顺序）；② 指向证据（`E2E闭环清单` `L-C3c` / `开发计划` §`PR-5b2`）。**没有新增义务**，不需要中台确认（与需要确认的 `PQ27` 不同）。 |
| 2026-09-13 | **§5.2 新增「推理档位」要求（`PR-5b1` 出方案时补，用户 2026-09-13 确认）。** 客户端要把用户的推理档位发到网关，才能让模型产生思考块（`L-C3b`），而**"网关接受哪个推理字段"此前从未写过**。新增内容三条：① 客户端发**扁平的顶层 `reasoning_effort`**（`low`／`medium`／`high`／`xhigh`／`max`），网关必须原样转发，**不得拒绝、不得改写、也不得静默忽略**（忽略的表现是"选了档位但思考块永不出现"，客户端分不清这与"该模型本就不推理"）；② 档位为关闭时**客户端完全不发该字段**，故"请求里没有它"是合法形态；③ **网关不得因出现自己暂时不认识的字段而返回 400** —— 客户端复用通用 OpenAI provider 栈，字段会随版本增减，"未知即拒"会把每次字段变化变成一次停机不兼容（与本节既有「标准 OpenAI 字段原样透传」同一条原则）。**为什么必须写死形状**：客户端 provider 用 `Dialect` 区分五种推理字段形状（`internal/provider/openai/client.go`），网关属哪种在写本节时**无人知道**；取标准形状，并在客户端把改动点收敛到一处。**登记为 `PQ27`（待中台确认）**。 |
| 2026-09-12 | **§4.3 义务表补一行「版本可比」（`PR-4b` 落地时发现）。** 客户端要对「回滚版本」做判定，就**必须先有一个版本次序**，而原表只写了"必须单调递增"、没说形状。落地时用字符串比较实现，**在同一天的第十个目录上就断**（`"…-09.10"` 字符串序**小于** `"…-09.9"` ⇒ 当天第十次刷新被判成回滚、静默拒绝刷新到第二天）。客户端已改为**按各段数值比较**，因此中台**不需要**定宽零填充；新增的义务只是"形状必须是 `YYYY-MM-DD[.<计数器>]` 且同一天计数器数值递增"。形状不匹配时客户端退回字符串比较，那**不保证**是版本次序 —— 这一条写在表内，因为它是对中台的要求，不是客户端的实现细节。 |
...
| 2026-09-11 | v0.9（P0-01 B0 落地时发现的登记表缺口）：① §3.2 补登记 `code_not_sent` / `activation_invalid` / `phone_mismatch`（400/403 行）—— 它们原本只在 §4.2.2 被引用，而 §3.2 自称唯一登记表，缺项会让前端映射表漏项；② §4.2.2 的 `code_invalid` / `too_many_requests` 与 §3.2 的 `invalid_code` / `rate_limited` 是同一件事的两套命名，已按 §3.2 收敛（违反 §3.8）；③ 本地 fail-closed 码补登记 `network_unavailable`（请求未到达中台，与中台 5xx 的 `upstream_unavailable` 区分），它同样不构成回落本地模型的理由。客户端侧**计划**有守卫：`internal/productclient/code_test.go` 的 `TestEveryCodeIsRegisteredInTheDoc`，断言每个 `Code` 常量都出现在本文件中。**（2026-09-12 更正：该测试当时并不存在，此句原写为"已有守卫"是假陈述 —— 见 `需求基线.md` `V-19`。守卫已在本轮补出，实测绿。）** |
| 2026-09-11 | v0.10（能力 ID 登记缺口）：新增 §4.4「能力 ID 注册表（初稿）」—— 此前 [P0-00A](需求基线.md) 承诺交付「capability ID 初稿」、[P0-04](需求基线.md) 要求「客户端先定义 capability ID 常量表」，但全目录没有这张表，`capabilities[].id` 只能靠 `"mcp"` 一个示例猜。现按当前仓库已有入口/执行面列出功能入口类与执行动作类两组 id，并声明 §4.4 为唯一登记表、Q4 只增删不改名。Q4 行同步指向 §4.4。 |
| 2026-09-11 | v0.11（boxCode 需求引入，属**合同变更**，需中台确认）：① §3.2 的 400 行新增登记 `activation_code_used` / `box_code_unknown` / `box_code_mismatch` —— 首次激活的第二个业务凭证 `boxCode` 需要可区分的失败语义，否则用户抄错一位就永久卡死（原因与客服兜底见 `需求基线` `E1` 规则 2、清单 `PQ9`）；② §4.2.2 登录请求体新增 `boxCode`（仅首次激活必填、与 `activationCode` 独立校验、两者一对多而激活码一次性），登录与 bootstrap 的 `activation` 对象新增 `boxCode`（**服务端是唯一真值来源**：全新 `data/` 上二次登录时客户端没有本地记录，只能由响应带回）；③ E2E-11 与 §10 `Q8` 的「复制 `data/` 不能复用登录态 / 凭证不可迁移」**改为「可复用登录态、凭证随 `data/` 迁移」** —— 便携 U 盘的产品定位要求「插上就能用」，代价是明文凭证可复制（用户明确接受，防拷贝登记为 `需求基线` `D-004`），安全性改由中台侧撤销 refresh token 兜底。 |
