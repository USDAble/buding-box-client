...
| Q12 | `/auth/refresh` 的**鉴权方式**与请求体形状；它是否也要 `Idempotency-Key` | refresh token 走**请求体 `{"refreshToken": …}`**，不带 `Authorization`（§4.1 该行"鉴权"列写的正是 `refresh token` 而非"是"，与 logout 不同）；按 §3.3"所有创建副作用的请求"同样带 `Idempotency-Key` | 认证链路能否联调；401 恢复路径（P0-02 A 的传输层） | 中台 | **P0-02 B（已在代码中按此默认实现，需中台书面确认）** |

**登记规则**（本节是 Q1–Q12 的唯一登记表；其它文档只引用，不复制）：
...
| 2026-09-11 | v0.11（P0-02 代码审计反查出的契约缺口）：新增 **Q12**。审计逐行核对了 `internal/productclient` 的认证实现与本文，发现两处本文**从未规定**的东西 —— ① `/auth/refresh` 的**鉴权方式**：§4.1 表里该行「鉴权」列写的是 `refresh token`（与 logout 的"是"不同），即 refresh token 走请求体而非 `Authorization` 头，但 §4.2.3 只说"接收 refresh token"，没写 body 形状；② 它是否也要 `Idempotency-Key`：§3.3 说"所有创建副作用的请求"，而代码里 refresh/logout 都标为 `CallWrite` 却没带该头。客户端已按 Q12 的推荐默认实现，**需中台书面确认**，否则联调会 400。§10 标题的范围同步由 Q1–Q11 改为 Q1–Q12。 |
